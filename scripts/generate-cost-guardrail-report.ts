...
const invalid = (): never => {
  throw new Error("Cost guardrail report arguments are invalid.");
};
...
export const generateCostGuardrailReportFiles = async ({
  asOf,
  inputPath,
  jsonOutputPath,
  markdownOutputPath,
  ...
  const report = projectCostGuardrailReport(input.value, { asOf, inputDigest });
  await writeExclusivePair(
    jsonOutputPath,
    `${JSON.stringify(report, null, 2)}\n`,
    markdownOutputPath,
    renderCostGuardrailMarkdown(report),
  );
  return report;
};

const main = async (): Promise<void> => {
  const report = await generateCostGuardrailReportFiles(parseArguments(process.argv.slice(2)));
  process.stdout.write(
    `Generated cost guardrail report with decision status ${report.decisionStatus}.\n`,
  );
};
