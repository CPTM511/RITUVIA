...
} from "./astrology-ai-release-evaluation.js";
...
const suitePath = path.join(root, "packages/ai/test/fixtures/astrology-release-evaluation-v1.json");
...
const testFiles = Object.freeze([
  "packages/ai/test/astrology-interpretation.test.ts",
  "tests/astrology-ai-release-evaluation.test.ts",
]);
...
const report = (() => {
  if (execution.error !== undefined || typeof execution.stdout !== "string") {
    return fail("Astrology AI evaluation runner failed before producing evidence.");
  }
  try {
    ...
    return fail("Astrology AI evaluation runner returned malformed evidence.");
  }
  return fail("Astrology AI evaluation runner returned malformed evidence.");
})();
...
const observations: readonly AstrologyReleaseEvaluationObservationV1[] = suite.cases.map(
  ({ assertionSuffix, id }) => {
...
const result = scoreAstrologyReleaseEvaluationV1({
  baseline,
  observations,
  suite,
  suiteChecksum,
