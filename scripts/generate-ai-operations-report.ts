...
const invalid = (): never => {
  throw new Error("AI operations report arguments are invalid.");
};
...
export const generateAiOperationsReportFiles = async ({
  asOf,
  inputPath,
  jsonOutputPath,
  markdownOutputPath,
  ...
  const report = projectAiOperationsReport(
    input.value,
    Object.freeze({
      asOf,
      inputDigest,
  ...
  await writeExclusivePair(
    ...
    markdownOutputPath,
    renderAiOperationsMarkdown(report),
  );
  return report;
};

const main = async (): Promise<void> => {
  const report = await generateAiOperationsReportFiles(parseArguments(process.argv.slice(2)));
  process.stdout.write(
    `Generated AI operations report with decision status ${report.decisionStatus}.\n`,
  );
};
