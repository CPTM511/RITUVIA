...
const routes = Object.freeze([
  Object.freeze({
    heading: "Western astrology, with the calculation kept visible",
    pathname: "/en/astrology",
    structuredType: "CollectionPage",
  }),
  Object.freeze({
    heading: "How RITUVIA calculates a natal chart",
    pathname: "/en/astrology/natal-chart-calculation",
    structuredType: "Article",
  }),
  Object.freeze({
    heading: "Birth-time uncertainty in natal charts",
    pathname: "/en/astrology/birth-time-uncertainty",
    structuredType: "Article",
  }),
  Object.freeze({
    heading: "Placidus houses and major aspects",
    pathname: "/en/astrology/houses-and-major-aspects",
    structuredType: "Article",
  }),
  Object.freeze({
    heading: "Astrology sources and calculation methodology",
...
const waitForServer = async (server) => {
  ...
  while (Date.now() < deadline) {
    if (server.exitCode !== null) {
      throw new Error("Astrology SEO browser server exited before ready.");
    }
    try {
      const response = await fetch(`${origin}/en/astrology`);
      if (response.ok) return;
    } catch {
      await new Promise((resolve) => setTimeout(resolve, 100));
    }
  }
  throw new Error("Astrology SEO browser server did not become ready.");
};
...
try {
  ...
  for (const route of routes) {
    ...
    assert.equal((await page.locator('a[href="/en/readings/astrology"]').count()) > 0, true);
    assert.equal(
      await page.evaluate(
    ...
    assert.equal(await page.getByText(/Owner-reviewed English method content/iu).count(), 1);
  ...
  for (const route of routes) {
    assert.equal(robots.includes(`Allow: ${route.pathname}$`), true);
  }
  assert.equal(robots.includes("/en/readings/astrology"), false);
  assert.equal(robots.includes("/en/astrology/aries"), false);

  const sitemapResponse = await context.request.get("/sitemaps/en-astrology.xml");
  ...
  assert.equal(sitemap.includes("/en/readings/astrology"), false);
  assert.equal(sitemap.includes("/en/astrology/aries"), false);
