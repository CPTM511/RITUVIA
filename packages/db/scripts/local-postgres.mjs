  ...
  try {
    ...
    if (commercialTransactionTables.rows[0]?.present === true) {
      ...
      if (commercialFulfillmentTables.rows[0]?.present === true) {
        ...
        await admin.query(
          `GRANT INSERT ON TABLE credit_ledger_entry, credit_projection, credit_restriction_entry, commercial_fulfillment_v2 TO ${PAYMENT_FULFILLMENT_ROLE}`,
        );
        await admin.query(
          `GRANT UPDATE (purchased_available, purchased_held, reserved, version, updated_at) ON TABLE credit_projection TO ${PAYMENT_FULFILLMENT_ROLE}`,
        );
        await admin.query(
          `GRANT UPDATE (status, held_amount, reversed_amount, shortfall_amount, applied_payment_state_version, last_outbox_id, version, updated_at) ON TABLE commercial_fulfillment_v2 TO ${PAYMENT_FULFILLMENT_ROLE}`,
        );
        await admin.query(
      ...
      if (commercialPaymentEventTables.rows[0]?.present === true) {
        ...
        await admin.query(
          `REVOKE UPDATE ON TABLE commercial_payment_outbox_v2 FROM ${PAYMENT_WEBHOOK_ROLE}`,
        );
      }
    }
    ...
    if (adminSecurityTables.rows[0]?.present === true) {
      ...
      const commerceAdminTables = await admin.query(
        "SELECT to_regclass('public.commerce_admin_operation_v1') IS NOT NULL AS present",
      );
      if (commerceAdminTables.rows[0]?.present === true) {
        await admin.query(
...
const createTestDatabase = async (runtime) => {
  ...
  const paymentFulfillmentDatabaseUrl = buildDatabaseUrl(
    runtime,
    databaseName,
    PAYMENT_FULFILLMENT_ROLE,
  );
  ...
  withLifecycleLock(async () => {
    ...
    return operation(
      Object.freeze({
        ...
        developmentPaymentFulfillmentDatabaseUrl: buildDatabaseUrl(
          runtime,
          DEVELOPMENT_DATABASE,
          PAYMENT_FULFILLMENT_ROLE,
        ),
